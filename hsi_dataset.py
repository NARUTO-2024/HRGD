...
class TrainDataset(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=True, stride=8):
        ...
        with open(f'{data_root}/split_txt/train_list.txt', 'r') as fin:
            hyper_list = [line.replace('\n','.mat') for line in fin if int(line.strip().split('_')[-1])]# hyper_list = [line.replace('\n','.mat') for line in fin if int(line.strip().split('_')[-1]) <= 900]  hyper_list = [line.replace('\n','.mat') for line in fin]
            bgr_list = [line.replace('mat','jpg') for line in hyper_list]
        hyper_list.sort()
        bgr_list.sort()
        print(f'len(hyper) of ntire2022 datase:{len(hyper_list)}')
        print(f'len(bgr) of ntire2022 datase:{len(bgr_list)}')
        for i in range(40):  #len(hyper_list)
...
class ValidDataset(Dataset):
    def __init__(self, data_root, crop_size, bgr2rgb=True, stride=8):
        self.crop_size = crop_size
        self.hypers = []
        self.bgrs = []
        h, w = 482, 512  # img shape
